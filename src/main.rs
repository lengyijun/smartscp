...
async fn main() -> Result<(), Error> {
    ...
    match direction {
        Direction::Upload => {
            upload(connection, &sftp).await;
            sftp.open("/tmp").await.unwrap().sync_all().await
        }
        Direction::Download => {
            let sess = get_remote_host(&remote_host).await.unwrap();
            download(connection, sess, sftp).await;
...
async fn download_dir(
    ...
    for remote_pf in v1.into_iter() {
        download_dir(c, sess, sftp, remote_pf.clone(), Some(blacklist.clone())).await;
    }
    join_all(v2).await;
    ...
    if c.local_path.is_dir() {
        match remote_dir_filestat {
            Ok(Some(stat)) => {
                ...
                sftp.fs().create_dir(&c.remote_path).await;
            }
            _ => {
